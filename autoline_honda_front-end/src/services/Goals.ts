...
const REST_API_BASE_URL = (cnpj: string) => `http://localhost:8080/goals/${cnpj}`;
...
export const createGoal = (goal: Goal) =>
  axios
    ...
    .catch((error) => console.error("Error creating goal:",
        error.response?.data || error.message)
    );

export const getGoals = (cnpj: string) =>
  axios.get(REST_API_BASE_URL(cnpj))
    .catch((error) => console.error("Error fetching goals:",
        error.response?.data || error.message)
    );

export const getGoal = (goalId: number) =>
  axios.get(`${REST_API_BASE_URL}/${goalId}`)
    .catch((error) => console.error("Error fetching goal:",
        error.response?.data || error.message)
    );
...
export const updateGoal = (
  ...
  axios
    ...
    .catch((error) => console.error("Error updating goal:",
      error.response?.data || error.message)
    );
